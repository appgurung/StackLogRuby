...
require "socket"
require "uri"
require_relative "stacklog/client.rb"
require_relative "stacklog/resources.rb"
require_relative "stacklog/version.rb"
...
module StackLog
  ...
  class Ruby < ::Logger
    ...
    def initialize(key, bucket_id, opts = {})
      ...
      url = "#{endpoint}#{bucket_id}"
      uri = URI(url)
      
      @client = StackLog::Client.new(uri, key, opts)
    end
    ...
    def add(*_arg)
      @internal_logger.debug("add not supported in StackLog logger")
      false
    end
    ...
    def datetime_format(*_arg)
      @internal_logger.debug("datetime_format not supported in StackLog logger")
      false
    end
